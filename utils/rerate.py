...
async def rewrite_message(message, client, stop_words=["|UGM|"]):
    ...
    # Удаляем стоп-слова из базы данных
    ...
    if media_list:
        # УБРАЛИ ОГРАНИЧЕНИЕ: text = rewrite_last_paragraph(text, 750)
        ...
        # УБРАЛИ ОГРАНИЧЕНИЕ: text = rewrite_last_paragraph(text, 3500)
...
async def send_to_channel(text, media_list, channel_username, telethon_client,
    ...
    try:
        ...
        async with Bot(token=bot_token) as bot:
            if media_paths:
                # УБРАЛИ ОГРАНИЧЕНИЕ: проверку длины текста для медиа
                # if text and len(text) > 750:
