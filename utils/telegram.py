...
# Импортируем AccountManager и AI Manager
...
async def AI_distributed(theme_context, messages):
    """Анализ сообщений с распределением по агентам"""
    try:
...
async def get_all_matches(desc_objects, messages, ignore_duplicates=False):
    """Получает все подходящие сообщения с контекстом темы ТБ"""
    ...
    # Фильтруем сообщения перед анализом
    valid_messages = []
    ...
    if not valid_messages:
        ...
        await account_manager.log_to_chat("⚠️ No valid messages for AI analysis", "WARNING")
        return []
    
    logger.info(f"Analyzing {len(valid_messages)} valid messages with distributed AI")
    await account_manager.log_to_chat(f"🤖 Starting distributed AI analysis of {len(valid_messages)} messages", "INFO")

    theme_context_parts = []
    
    # Обрабатываем список или одиночный объект
    tb_objects = desc_objects if isinstance(desc_objects, list) else [desc_objects]
    
    for i, tb_obj in enumerate(tb_objects, 1):
        if len(tb_objects) > 1:
            theme_context_parts.append(f"--- ТЕМАТИЧЕСКИЙ БЛОК {i} ---")
        ...
        if len(tb_objects) > 1:
            theme_context_parts.append("")  # Пустая строка между ТБ
    ...
    # Используем новую распределенную систему анализа с контекстом темы
...
async def copy_posts(
        ...
        for item in desc:
            ...
            for match in matches:
                ...
                if match['ratio'] >= 0.85:  
                    ...
                    if success:
                        published = True
                        break  # Выходим при успешной публикации
                        
            if published or attempts >= max_attempts: 
                break  # Выходим из внешнего цикла если опубликовали или достигли лимита
    else:
    ...
    if published:
        ...
        await account_manager.log_to_chat(f"✅ Successfully published after {attempts} attempts", "SUCCESS")
    elif attempts >= max_attempts:
        logger.error(f"Failed to publish: reached maximum attempts ({max_attempts})")
        await account_manager.log_to_chat(f"❌ Publication failed: max attempts reached ({max_attempts})", "ERROR")
    else:
        logger.warning(f"No suitable messages found for publication with rating >= 85% after {attempts} attempts")
        await account_manager.log_to_chat(f"⚠️ No suitable messages found after {attempts} attempts", "WARNING")
...
async def main(id, ignore_duplicates=False):
    try:
        ...
        if publication_result:
            ...
            await account_manager.log_to_chat(f"❌ Failed to complete TB {display_id} processing", "ERROR")
...
async def initialize_ai_manager():
    ...
    try:
        ...
        if success:
            ...
            await account_manager.log_to_chat("🤖 AI Manager initialized with distributed processing", "SUCCESS")
        else:
            logger.error("Failed to initialize AI Manager")
            await account_manager.log_to_chat("❌ Failed to initialize AI Manager", "ERROR")
        return success
    except Exception as e:
        logger.error(f"Error initializing AI Manager: {e}")
        await account_manager.log_to_chat(f"❌ AI Manager error: {str(e)[:200]}", "ERROR")
        return False
