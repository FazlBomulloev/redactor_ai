...
def safe_get_block_names(blocks):
    """Безопасно получает имена блоков, обрабатывая None значения"""
    if not blocks:
        return "Блоки не найдены"
    ...
    for block in blocks:
        if block is None:
            ...
            block_names.append("Блок без имени")
        elif not block.name:
            block_names.append("Блок с пустым именем")
        else:
            block_names.append(block.name)
    
    return ", ".join(block_names) if block_names else "Нет доступных блоков"


async def get_thematic_block_display(tb_id_string):
    """Безопасно получает отображение тематических блоков"""
    try:
        if not tb_id_string or str(tb_id_string).strip() == "" or str(tb_id_string).strip() == "0":
            return "Пустое время"
        
        # Разбиваем строку на ID
...
async def get_publication_display(pub_id):
    ...
    try:
        ...
        return "Ошибка загрузки публикации"
...
async def publication_data(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        for pb in current_page_pb:
            ...
            # Безопасное получение отображения
            display_text = "Пустое время"
            
            # Проверяем тематические блоки
            ...
            if display_text in ["Блоки не найдены", "Ошибка загрузки блоков"] and pb.ind_pub_id:
...
async def publication_detail(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        # Проверяем, является ли это пустым временем
        is_empty_time = (
...
async def publication_data(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        if "tb" in data:
            ...
            for pb in current_page_pb:
                # Безопасная проверка имени блока
...
async def select_publications_fl(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        kb = InlineKeyboardBuilder()
        for pb in current_page_pb:
            # Безопасная проверка имени публикации
            pub_name = "Неизвестная публикация"
            if pb and hasattr(pb, 'name') and pb.name:
                pub_name = pb.name
